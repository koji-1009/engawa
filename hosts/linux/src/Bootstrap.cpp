...
// async results back. evaluate cannot await a page promise, so the page reports results over the same
// webkit message-handler channel, tagged `__conf` so the host can tell a control-relay message apart
...
const char* const kConformanceRelay = R"JS(
  var post = function(m){ window.webkit.messageHandlers.engawa.postMessage(JSON.stringify(m)); };
  window.__engawaConf = {
...
std::string build(const std::string& platform, const std::string& contractVersion,
    ...
    s += "    return;\n";
    s += "  }\n";
    // §1: the real bridge. postMessage rides WebKitGTK's script-message-handler channel
    // (window.webkit.messageHandlers.engawa), which is not governed by the page CSP.
    s += "  window.__shell = {\n";
    s += "    contractVersion: " + ver + ",\n";
    s += "    platform: " + plat + ",\n";
    s += "    capabilities: " + caps + ",\n";
    s += "    postMessage: function(s){ window.webkit.messageHandlers.engawa.postMessage(s); }\n";
    s += "  };\n";
