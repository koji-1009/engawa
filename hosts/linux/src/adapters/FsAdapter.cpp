...
// must be absolute (spec/commands/fs.md). On Linux an absolute path begins with '/'.
...
double modifiedMs(const std::string& p) {
    ...
    return static_cast<double>(st.st_mtim.tv_sec) * 1000.0 + static_cast<double>(st.st_mtim.tv_nsec) / 1e6;
}
