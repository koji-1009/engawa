#pragma once
// Builds the document-start script injected via WebView2's user-script path
// (AddScriptToExecuteOnDocumentCreated) — the "evaluate a string" companion to postMessage. That
...
//
// WebView2 runs a document-start script in EVERY frame, so the §6 guard lives inside the script:
