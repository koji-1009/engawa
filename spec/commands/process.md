...
| `process.read` | `{ pid, stream, maxBytes }` | `{ data, eof }` | Drains up to `maxBytes` bytes from `stream` (`"stdout"`/`"stderr"`), decoded as UTF-8 without splitting a multibyte sequence. `eof` is true once the process has exited and the stream is fully drained. Unknown `pid` → `ESRCH`. |
