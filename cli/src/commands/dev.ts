...
export async function cmdDev(argv: string[]): Promise<void> {
  ...
  if (process.platform === "linux") return devLinux(argv);
...
// Linux has no .app bundle: build the native host, then launch EngawaHost in place with the app's
...
async function devLinux(argv: string[]): Promise<void> {
