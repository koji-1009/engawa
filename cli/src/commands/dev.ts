...
export async function cmdDev(argv: string[]): Promise<void> {
  if (process.platform === "win32" || process.platform === "linux") return devNative(argv);
...
// Windows/Linux have no .app bundle: build the native host, then launch it in place with the app's
// assets/shell.js/data wired through the environment (the same variables the make-notes gate uses). A
// normal (non-conformance) launch shows the window.
async function devNative(argv: string[]): Promise<void> {
